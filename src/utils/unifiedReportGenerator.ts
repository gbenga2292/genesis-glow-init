...
export const generateUnifiedReport = async (config: ReportConfig): Promise<void> => {
  ...
  */
  ...
  // ── Section below separator ───────────────────────────────────────────────
  let cursorY = separatorY + 8;

  // Subtitle / location: left-aligned, ABOVE the main title
  if (subtitle) {
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
  ...
  // Generated on / Report type — small grey metadata
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(100, 100, 100);
  ...
  if (reportType) {
    doc.text(`Report Type: ${reportType.toUpperCase()}`, margin, cursorY);
    cursorY += 5;
  }
